      ...
      users: [
         {
            ...
            notes: {
               ...
               conclusion:
                  'Bạn phù hợp với người có xu hướng ổn định và thấu cảm...',
            },
         },
         {
            name: 'Nguyễn Thị B',
...
export default function CoupleReportPage({ params }: CoupleReportPageProps) {
   ...
   return (
      <div className="max-w-6xl mx-auto p-6 bg-white rounded-lg shadow-md">
         ...
         <div className="grid grid-cols-2 gap-6 bg-pink-50 p-6 rounded-lg mb-8 text-gray-800 border border-primary/20">
            {users.map((user, i) => (
               <div key={i}>
                  ...
                  <p>
                     <span className="font-semibold">Ngày tư vấn:</span> {date}
                  </p>
                  <p>
                     <span className="font-semibold">Chuyên viên:</span>{' '}
                     {expert}
                  </p>
                  <p>
                     <span className="font-semibold">MBTI:</span> {user.mbti}
                  </p>
                  <p>
                  ...
                  <p>
                     <span className="font-semibold">DISC:</span> {user.disc}
                  </p>
               </div>
            ))}
         </div>
         ...
         {/* MBTI */}
         <ReportSection
            title="MBTI – Phân tích tính cách"
            summary="So sánh MBTI của hai người"
         >
         ...
         <ReportSection
            ...
            summary="So sánh ngôn ngữ tình yêu của hai người"
         >
            <div className="grid grid-cols-2 gap-6">
               {users.map((user, i) => (
                  <div key={i}>
                     <h3 className="font-semibold text-lg mb-2">
                        {user.name} – {user.loveLang}
                     </h3>
         ...
         {/* Big5 */}
         <ReportSection
            title="Big5 – Phân tích tính cách chi tiết"
            summary="So sánh các đặc điểm tính cách theo mô hình Big5"
         >
            <div className="grid grid-cols-2 gap-6">
               {users.map((user, i) => (
                  <div key={i}>
         ...
         {/* DISC */}
         <ReportSection
            title="DISC – Phân tích hành vi"
            summary="So sánh phong cách hành vi DISC của hai người"
         >
            <div className="grid grid-cols-2 gap-6">
               {users.map((user, i) => (
                  <div key={i}>
         ...
         <div className="mt-10 bg-primary/10 border border-primary rounded-md p-6">
            <h2 className="text-xl font-semibold mb-4 text-primary">
               Kết luận & Định hướng cho cặp đôi
            </h2>
            {users.map((user, i) => (
