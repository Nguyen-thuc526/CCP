...
import { ArrowLeft, CheckCircle, XCircle, Clock, Calendar, User, Tag, FileText, AlertCircle } from "lucide-react"
...
export default function CertificateDetailPage() {
  const certificate = useSelector((state: RootState) => state.certificate.selectedCertificate)
  ...
  const handleApprove = async () => {
    ...
    try {
      ...
      showToast(`Đã xảy ra lỗi khi duyệt chứng chỉ: ${(error as Error).message}`, ToastType.Error);
    } finally {
  ...
  if (!certificate) {
    return (
      <div className="container mx-auto px-4 py-8">
        <p className="text-center text-muted-foreground">Không tìm thấy dữ liệu chứng chỉ.</p>
      </div>
    )
  }
  return (
    <>
      ...
      <div className="mb-6">
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
          ...
          <Badge className={`${getStatusColor(certificate.status)} flex items-center gap-2 w-fit`}>
      ...
      <div className="grid gap-6 lg:grid-cols-3">
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <Card>
            ...
            <CardContent>
              <p className="text-sm leading-relaxed whitespace-pre-wrap">{certificate.description}</p>
            </CardContent>
          </Card>
        ...
        <div className="space-y-6">
            ...
            <CardContent className="space-y-4">
              ...
              <div>
                ...
                <div className="flex items-center gap-2 mt-1">
                  {getStatusIcon(certificate.status)}
                  <span className="font-medium">{getStatusText(certificate.status)}</span>
                </div>
              </div>
            </CardContent>
          </Card>
          ...
          {/* Actions */}
          {[CertificateStatus.NeedEdit, CertificateStatus.Pending].includes(certificate.status) && (
            <Card>
              <CardHeader>
                <CardTitle>Hành động</CardTitle>
                <CardDescription>Duyệt hoặc từ chối chứng chỉ này</CardDescription>
              </CardHeader>
              <CardContent className="space-y-4">
