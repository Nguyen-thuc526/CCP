...
export const CourseService = {
  ...
  async getChapterDetail(id: string): Promise<ChapterDetail> {
    const response = await axiosInstance.get(`/api/Course/${id}/chapter-detail`);
    return response.data.data;
  },

  async createLecture(data: CreateLectureRequest): Promise<CreateLectureResponse> {
    const response = await axiosInstance.post("/api/Course/create-lecture", data);
    return response.data;
  },


  async createQuiz(data: CreateQuizRequest): Promise<CreateQuizResponse> {
    const response = await axiosInstance.post("/api/Course/create-quiz", data);
    return response.data;
  },

  async createQuestion(data: CreateQuestionRequest): Promise<CreateQuestionResponse> {
    const response = await axiosInstance.post("/api/Course/create-question", data);
    return response.data;
  },
  ...
  async deleteQuestion(questionId: string): Promise<{ success: boolean }> {
    const response = await axiosInstance.delete(`/api/Course/delete-question/${questionId}`);
    return response.data;
  },

  async updateQuestion(data: UpdateQuestionRequest): Promise<{ success: boolean }> {
    const response = await axiosInstance.put("/api/Course/update-question", data);
    return response.data;
  },
  async addSubCategoryToCourse(
    data: AddSubCategoryToCourseRequest
  ): Promise<AddSubCategoryToCourseResponse> {
    const response = await axiosInstance.post("/api/Course/add-subcate", data);
    return response.data;
  },
  async createVideo(data: CreateVideoRequest): Promise<CreateVideoResponse> {
    const response = await axiosInstance.post("/api/Course/create-video", data);
    return response.data;
  },
  async updateCourse(data: UpdateCourseRequest): Promise<UpdateCourseResponse> {
    const response = await axiosInstance.post("/api/Course/update-course", data);
    return response.data;
  },
  async updateLecture(data: UpdateLectureRequest): Promise<CreateLectureResponse> {
  ...
  async changeCourseStatus(data: { courseId: string, newStatus: number }): Promise<{ success: boolean }> {
