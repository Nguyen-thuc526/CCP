      ...
      <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50">
         <Card className="w-[400px]">
            <CardHeader>
               ...
               <CardDescription>
                  Vui lòng gửi đánh giá cho cuộc gọi với ID: {roomName}
               </CardDescription>
            </CardHeader>
            <CardContent>
               <p className="text-sm text-muted-foreground">
                  Form đánh giá sẽ được thêm vào đây.
               </p>
            </CardContent>
...
export function VideoRoom({
   ...
   useEffect(() => {
      if (!token) {
         ...
         setError(
            'Không thể kết nối đến phòng họp do thiếu token. Vui lòng thử lại.'
         );
         setIsConnecting(false);
      } else if (!roomName || !participantName) {
         console.error('Error: Missing roomName or participantName', {
            roomName,
            participantName,
         });
         setError(
            'Thông tin phòng hoặc người tham gia không hợp lệ. Vui lòng thử lại.'
         );
         setIsConnecting(false);
      } else if (!serverUrl) {
            ...
            <Card className="w-[400px]">
               <CardHeader>
                  ...
                  <CardDescription className="text-center">
                     Đang thiết lập kết nối đến phòng họp
                  </CardDescription>
               </CardHeader>
   ...
   if (error || !token || !serverUrl) {
      console.error('VideoRoom error state:', {
         error,
         token: token ? 'Token present' : 'Token missing',
         serverUrl: serverUrl ? 'ServerUrl present' : 'ServerUrl missing',
      });
      return (
         <div className="flex min-h-screen items-center justify-center bg-background">
            <Card className="w-[400px]">
               <CardHeader>
                  ...
                  <CardDescription className="text-center">
                     {error || 'Không thể kết nối đến phòng họp.'}
                  </CardDescription>
               </CardHeader>
               <CardContent className="flex justify-center py-6">
               ...
               <CardFooter className="flex justify-center">
                  <Button onClick={() => router.push('/mybooking')}>
                     Quay lại
                  </Button>
               </CardFooter>
            </Card>
