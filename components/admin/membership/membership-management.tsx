    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">Quản lý các gói membership theo cấp bậc</p>
        </div>
          ...
          <DialogContent className="max-w-2xl">
            ...
            <form onSubmit={formik.handleSubmit} className="space-y-4">
              ...
              <div className="flex justify-end gap-2 pt-2">
                <Button variant="outline" type="button" onClick={() => setShowCreateDialog(false)}>Hủy</Button>
      ...
      {!loading && !error && (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          {membershipTiers.sort((a, b) => a.rank - b.rank).map((membership) => (
            <Card key={membership.id} className="relative overflow-hidden border">
              ...
              <div className="absolute top-0 right-0 w-0 h-0 border-l-[30px] border-l-transparent border-t-[30px] border-t-primary/20">
                <span className="absolute -top-6 -right-2 text-xs font-bold text-primary">{membership.rank}</span>
              </div>
            </Card>
          ))}
        </div>
      ...
      <Card>
        ...
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-6 gap-4">
            <Stat label="Tổng số gói" value={membershipTiers.length} color="blue" />
            <Stat
              label="Tổng giá trị các gói"
              value={formatCurrency(membershipTiers.reduce((sum, tier) => sum + tier.price, 0))}
              color="green"
            />
            <Stat
            ...
            <Stat
              ...
              value={membershipTiers.length > 0 ? `${Math.max(...membershipTiers.map(t => t.discountCourse))}%` : "—"}
              color="orange"
            />
            <Stat
              label="Giảm đặt lịch cao nhất"
              value={membershipTiers.length > 0 ? `${Math.max(...membershipTiers.map(t => t.discountBooking))}%` : "—"}
              color="pink"
            />
            <Stat
              label="Thời hạn dài nhất"
              value={membershipTiers.length > 0 ? `${Math.max(...membershipTiers.map(t => t.expiryDate))} ngày` : "—"}
              color="cyan"
            />
          </div>
        </CardContent>
