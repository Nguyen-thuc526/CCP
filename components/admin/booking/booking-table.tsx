...
export function BookingTable({ bookings }: BookingTableProps) {
  ...
  return (
    <>
              ...
              <TableRow key={booking.id}>
                ...
                <TableCell>
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      <p className="font-medium text-sm">{booking.member.fullname}</p>
                      {booking.member2 && <p className="text-xs text-gray-500">+ {booking.member2.fullname}</p>}
                    </div>
                  </div>
                </TableCell>
                <TableCell>
                  <div className="flex items-center gap-2">
                    <Avatar className="h-8 w-8">
                      ...
                      <AvatarFallback>{booking.counselor.fullname.charAt(0)}</AvatarFallback>
                    </Avatar>
                    <div>
                      <p className="font-medium text-sm">{booking.counselor.fullname}</p>
                      <p className="text-xs text-gray-500">{booking.counselor.specialization}</p>
                    </div>
                  </div>
                </TableCell>
                <TableCell>
                  <div className="text-sm">
                    ...
                    <p className="text-gray-500">đến {format(new Date(booking.timeEnd), "HH:mm")}</p>
                  </div>
                </TableCell>
                <TableCell>
      ...
      <BookingDetailsModal booking={selectedBooking} open={isModalOpen} onOpenChange={setIsModalOpen} />
    </>
  )
}
