...
export default function CreateCategoryForm({ onCreated, onClose, categories }: Props) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const isDuplicate = categories.some((category) => category.name.trim().toLowerCase() === trimmedName.toLowerCase())
    ...
    try {
      ...
      setName("")
      onCreated() // This will trigger API fetch in parent
    } catch (error) {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      <div>
        ...
        <Input placeholder="Nhập tên danh mục..." value={name} onChange={(e) => setName(e.target.value)} />
      </div>
