...
export default function SurveyManagement() {
  ...
  const { loading, error, startLoading, stopLoading, setErrorMessage, renderStatus } =
  ...
  const refreshSurveys = async () => {
    try {
      ...
      setErrorMessage(
        err instanceof Error ? err.message : "Có lỗi xảy ra khi tải lại dữ liệu"
      )
    } finally {
      stopLoading()
    }
  ...
  return (
    <div className="p-6">
      ...
      <div className="mb-6">
        ...
        <p className="text-gray-600">Quản lý câu hỏi và câu trả lời cho các bài đánh giá tính cách</p>
      </div>
      ...
      {!loading && !error && surveys.length === 0 && (
        <div className="text-center py-12">
          <p className="text-gray-500">Không có khảo sát nào được tìm thấy.</p>
        </div>
      )}
