...
import {
  comparePersonType,
  updateResultPersonType,
} from '@/services/personTypeService';
...
export default function PersonalityComparison({
    ...
    try {
      ...
      if ((Array.isArray(result) ? result : [result]).length > 0) {
        setCurrentPersonType(
          (Array.isArray(result) ? result : [result])[0].personType
        );
      }
    } catch (err) {
      const errorMessage =
  ...
  const categories = Array.from(
    new Set(data.map((item) => item.category?.name).filter(Boolean))
  );
          ...
          <Card className="border border-gray-200">
            <CardContent className="text-center py-12">
              ...
              <p className="text-gray-600 mb-6">
                Không thể tải thông tin so sánh tính cách
              </p>
              ...
              <div className="grid md:grid-cols-3 gap-8 items-center">
                ...
                <div className="text-center space-y-4">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-gray-500 mt-1">Độ Tương Thích</p>
                  </div>
                </div>
            ...
            selectedComparison.weaknesses) && (
              <Card className="border border-gray-200 shadow-sm rounded-xl">
                ...
                <CardContent className="space-y-6">
                  ...
                  {selectedComparison.weaknesses && (
                    <section>
                      <h4 className="font-semibold text-red-700 mb-2">Điểm yếu</h4>
                      <div
              ...
              <div className="grid grid-cols-1 md:grid-cols-4 gap-6 text-sm">
                {[
                  ...
                  },
                  {
                    ...
                    label: 'Ngày tạo',
                    value: new Date(selectedComparison.createAt).toLocaleDateString('vi-VN'),
                  },
                  {
                    ...
                    value: (
                      <Badge
                        variant={selectedComparison.status === 1 ? 'default' : 'secondary'}
                        className="mt-1"
                      >
                  ...
                  <div key={idx} className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p
                        className={`text-gray-600 text-xs ${item.mono ? 'font-mono' : ''
                          }`}
                      >
                        {item.value}
                      </p>
      ...
      <div className="max-w-7xl mx-auto p-6 space-y-6">
        ...
        <div className="space-y-4">
          ...
          <p className="text-gray-600">
            Khám phá mức độ tương thích với các loại tính cách khác
          </p>
        </div>
            ...
            <CardContent>
              <div className="flex items-center gap-6">
                <div className="w-16 h-16">
                  <Image
                    src={
                      currentPersonType.image ||
                      '/placeholder.svg?height=64&width=64&query=current-mbti'
                    }
                    alt={
                      currentPersonType.name || 'Current Person Type'
                    }
                    width={64}
                    height={64}
                    className="rounded-full object-cover border-2 border-gray-200"
                  ...
                  <CardContent className="p-6">
                    ...
                    <div className="flex items-center justify-center mb-4">
                      <div className="flex items-center gap-2 text-sm text-gray-600">
                        <span>
                          {currentPersonType?.name?.split(
                            ' - '
                          )[0] || 'Hiện tại'}
                        </span>
                        <ArrowRight className="w-4 h-4" />
                        <span className="font-medium text-gray-900">
                          {comparison.personType2?.name?.split(
                            ' - '
                          )[0] || 'Không xác định'}
                        </span>
                      </div>
                    ...
                    <div className="text-center mb-4">
                      <div className="w-16 h-16 mx-auto mb-3">
                        <Image
                          src={
                            comparison.personType2?.image ||
                            '/placeholder.svg?height=64&width=64&query=target-mbti'
                          }
                          alt={
                      ...
                      <p className="text-xs text-gray-600 line-clamp-2">
                        {comparison.personType2?.description || ''}
                      </p>
                    </div>
