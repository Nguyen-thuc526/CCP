...
export default function CertificatesPage() {
  ...
  const filteredCertificates = useMemo(() => {
    return certificates.filter((cert) => {
      ...
      const matchesCategory =
        ...
        cert.categories.some((cat) => cat.categoryName === selectedCategory)
  ...
  return (
    <>
      <div className="mb-8">
        <h1 className="text-3xl font-bold mb-2">Quản lý Chứng chỉ</h1>
        <p className="text-muted-foreground">Danh sách các chứng chỉ chờ duyệt</p>
      </div>
      ...
      <div className="mb-6">
        <p className="text-sm text-muted-foreground">
          Hiển thị {filteredCertificates.length} trong tổng số {certificates.length} chứng chỉ
        </p>
      </div>
          ...
          <Card key={certificate.id} className="overflow-hidden hover:shadow-lg transition-shadow">
            ...
            <CardHeader className="pb-3">
              <div className="flex items-start justify-between gap-2">
                <CardTitle className="text-lg line-clamp-2">{certificate.name}</CardTitle>
            ...
            <CardContent className="pt-0">
              <CardDescription className="line-clamp-3 mb-4">{certificate.description}</CardDescription>
              ...
              <div className="space-y-3">
                <div>
                  ...
                  <div className="flex flex-wrap gap-1">
                    {certificate.categories.map((category) => (
                      <Badge key={category.categoryId} variant="secondary" className="text-xs">
                ...
                <div className="flex justify-between items-center">
                  <span className="text-sm text-muted-foreground">ID: {certificate.id.slice(-8)}</span>
                  <Button
                    size="sm"
                    variant="outline"
                    ...
                    onClick={() => {
                      ...
                      router.push(`/admin/certificate-management/${certificate.id}`)
                    }}
                  >
