                        ...
                        <div className="relative w-full overflow-auto">
                            <table className="w-full caption-bottom text-sm">
                                <thead className="border-b">
                                    <tr>
                                        <th className="h-12 px-4 text-left font-medium">Chuyên viên</th>
                                        <th className="h-12 px-4 text-left font-medium">Số điện thoại</th>
                                        <th className="h-12 px-4 text-left font-medium">Kinh nghiệm</th>
                                ...
                                <tbody>
                                    {counselors.map((counselor) => (
                                        <tr key={counselor.id} className="border-b transition-colors hover:bg-muted/50">
                                            <td className="p-4">
                                                <div className="flex items-center gap-3">
                                                    <Avatar>
                                                        ...
                                                        />
                                                        <AvatarFallback>{counselor.fullname[0]}</AvatarFallback>
                                                    </Avatar>
                                                    <div>
                                            ...
                                            <td className="p-4">{counselor.yearOfJob} năm</td>
                                            <td className="p-4">{getStatusBadge(counselor.status)}</td>
                                            <td className="p-4 flex gap-2">
                                                {counselor.status !== CounselorStatus.NoFunc && (
                                                    <div className="flex gap-2">
                                                        <Button
                                                            variant="ghost"
                                                            size="sm"
                                                            disabled={counselor.status === CounselorStatus.Block}
                                                            onClick={() =>
                                                                handleStatusChange(counselor.id, CounselorStatus.Block)
                                                            }
                                                        >
                                                        ...
                                                        <Button
                                                            variant="ghost"
                                                            size="sm"
                                                            disabled={counselor.status === CounselorStatus.Active}
                                                            onClick={() =>
                                                                handleStatusChange(counselor.id, CounselorStatus.Active)
                                                            }
                                                        >
