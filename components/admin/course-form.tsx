...
export function CourseForm({ courseId }: CourseFormProps) {
    ...
    const handleMembershipChange = (membershipId: string, checked: boolean) => {
        if (checked) {
            ...
            setFormData((prev) => ({
                ...prev,
                membershipRequired: prev.membershipRequired.filter((id) => id !== membershipId),
            }))
        }
    }
    ...
    const addChapter = () => {
        if (
            ...
            const chapter: Chapter = {
                ...
                videoUrl: newChapter.videoFile ? URL.createObjectURL(newChapter.videoFile) : undefined,
                                ...
                                <div className="space-y-2">
                                    {membershipOptions.map((option) => (
                                        <div key={option.id} className="flex items-center space-x-2">
                                            <Checkbox
                                                id={option.id}
                                                checked={formData.membershipRequired.includes(option.id)}
                                    ...
                                    <div className="flex gap-2">
                                        {formData.membershipRequired.map((membership) => (
                                            <Badge key={membership} variant="outline">
                                                {membershipOptions.find((opt) => opt.id === membership)?.label}
                                            </Badge>
                                        ))}
                                    </div>
                                )}
                        ...
                        <Card>
                            ...
                            <CardContent className="space-y-4">
                                <div className="grid gap-4 md:grid-cols-2">
                                    ...
                                    <div className="space-y-2">
                                        <Label htmlFor="chapter-type">Loại nội dung</Label>
                                        <Select
                                        ...
                                        >
                                            ...
                                            <SelectContent>
                                                ...
                                                <SelectItem value="quiz">Quiz</SelectItem> {/* Thêm lựa chọn Quiz */}
                                            </SelectContent>
                                        </Select>
                                    </div>
                                </div>
                                ...
                                {newChapter.type === "video" ? (
                                    <div className="space-y-4">
                                        ...
                                        <div className="space-y-2">
                                            <Label htmlFor="duration">Thời lượng (phút:giây)</Label>
                                            <Input
                                                id="duration"
                                    ...
                                    <div className="space-y-4">
                                        <div className="space-y-2">
                                            <Label htmlFor="chapter-content">Nội dung bài viết</Label>
                                            <Textarea
                                                id="chapter-content"
                                        ...
                                        <div className="space-y-2">
                                            <Label htmlFor="read-time">Thời gian đọc (phút)</Label>
                                            <Input
                                                id="read-time"
                                    ...
                                    <div className="space-y-4">
                                        ...
                                        {newChapter.quiz.length === 0 && (
                                            <p className="text-sm text-muted-foreground">Chưa có câu hỏi nào được thêm</p>
                                        )}
                                    </div>
                                )}
                                        ...
                                        <div key={chapter.id} className="flex items-start gap-4 p-4 border rounded-lg">
                                            ...
                                            <div className="flex-1 space-y-2">
                                                ...
                                                {chapter.type === "video" && chapter.videoUrl && (
                                                    <div className="relative">
                                                        <video src={chapter.videoUrl} controls className="w-full max-w-sm h-32 rounded border" />
                                                    </div>
                                                )}
                                                ...
                                                <div className="text-sm text-muted-foreground">
                                                    {chapter.type === "video" ? chapter.duration : `${chapter.readTime} phút`}
                                                </div>
                                            </div>
