...
import { Check, X, Clock, User, CreditCard, Calendar, DollarSign } from "lucide-react"

interface WithdrawItemProps {
    withdraw: Withdraw
    onUpdateStatus: (id: string, status: WithdrawStatus, reason?: string) => void
    isUpdating: boolean
}

export function WithdrawItem({ withdraw, onUpdateStatus, isUpdating }: WithdrawItemProps) {
        ...
        <Card className="w-full">
            <CardHeader className="pb-3">
                <div className="flex items-center justify-between">
                    <CardTitle className="text-lg">#{withdraw.id.slice(-8)}</CardTitle>
            ...
            <CardContent className="space-y-4">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div className="space-y-2">
                        <div className="flex items-center gap-2">
                            ...
                            <span className="font-semibold text-lg">{formatCurrency(withdraw.total)}</span>
                        </div>
                        <div className="flex items-center gap-2">
                ...
                {withdraw.status === WithdrawStatus.PendingWithdrawal && (
                    <div className="flex gap-2 pt-2">
                        <Button
                            onClick={() => onUpdateStatus(withdraw.id, WithdrawStatus.Approved)}
                            disabled={isUpdating}
                        ...
                        <Dialog open={showRejectDialog} onOpenChange={setShowRejectDialog}>
                            <DialogTrigger asChild>
                                <Button variant="destructive" disabled={isUpdating} className="flex-1">
                            ...
                            <DialogContent>
                                <DialogHeader>
                                    ...
                                    <DialogDescription>Vui lòng nhập lý do từ chối yêu cầu rút tiền này.</DialogDescription>
                                </DialogHeader>
                                <div className="space-y-2">
