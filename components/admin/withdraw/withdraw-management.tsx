...
export default function WithdrawManagement() {
    const { withdraws, loading, updating, fetchWithdrawsByStatus, updateStatus } = useWithdrawStore()
    const [activeTab, setActiveTab] = useState<string>(WithdrawStatus.PendingWithdrawal.toString())
                ...
                <Card>
                    <CardContent className="flex flex-col items-center justify-center py-12">
                        <div className="text-muted-foreground text-center">
                            ...
                            <h3 className="text-lg font-medium mb-2">Không có yêu cầu rút tiền</h3>
                            <p className="text-sm">Chưa có yêu cầu rút tiền nào trong trạng thái này.</p>
                        </div>
                    </CardContent>
                </Card>
            )
        ...
        <div className="container mx-auto py-6 space-y-6">
            <div className="flex items-center justify-between">
                <div>
                    ...
                    <p className="text-muted-foreground">Quản lý các yêu cầu rút tiền từ người dùng</p>
                </div>
            ...
            <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
                <TabsList className="grid w-full grid-cols-3">
                    {Object.values(WithdrawStatus)
                        ...
                        .map((status) => {
                            ...
                            return (
                                <TabsTrigger key={status} value={status.toString()} className="flex items-center gap-2">
                                    <Icon className={`w-4 h-4 ${tabInfo.color}`} />
                                    {tabInfo.label}
                                    <Badge
                                        className={`ml-1 ${tabInfo.color === "text-yellow-600"
                                                ? "bg-yellow-600"
                ...
                {Object.values(WithdrawStatus)
                    .filter((status) => typeof status === "number")
                    .map((status) => (
                        <TabsContent key={status} value={status.toString()} className="mt-6">
