                        ...
                        <div className="relative w-full overflow-auto">
                            <table className="w-full caption-bottom text-sm">
                                <thead className="border-b">
                                    <tr>
                                        <th className="h-12 px-4 text-left font-medium">Thành viên</th>
                                        <th className="h-12 px-4 text-left font-medium">SĐT</th>
                                        <th className="h-12 px-4 text-left font-medium">Ngày sinh</th>
                                ...
                                <tbody>
                                    {members.map((member) => (
                                        <tr key={member.id} className="border-b transition-colors hover:bg-muted/50">
                                            ...
                                            <td className="p-4">{formatDate(member.dob)}</td>
                                            <td className="p-4">{getStatusBadge(member.status)}</td>
                                            <td className="p-4 flex gap-2">
                                                <Button
                                                    variant="ghost"
                                                    size="sm"
                                                    disabled={member.status === AccountStatus.Block}
                                                    onClick={() => handleStatusChange(member.id, AccountStatus.Block)}
                                                >
                                                ...
                                                <Button
                                                    variant="ghost"
                                                    size="sm"
                                                    disabled={member.status === AccountStatus.Active}
                                                    onClick={() => handleStatusChange(member.id, AccountStatus.Active)}
                                                >
