...
import type { CourseItem, SubCategory, ChapterItem, UpdateCourseRequest } from "@/types/course"
...
export function AdminCourseDetail({ courseId }: AdminCourseDetailProps) {
  ...
  const sortChaptersByCreatedAt = (chs: Chapter[]) =>
    [...chs].sort((a, b) => new Date(a.createdAt).getTime() - new Date(b.createdAt).getTime())
  // Temp states for basic info & pricing
  const [tempTitle, setTempTitle] = useState("")
  const [tempDescription, setTempDescription] = useState("")
  const [tempThumbnail, setTempThumbnail] = useState("")
  const [tempThumbnailFile, setTempThumbnailFile] = useState<File | null>(null)
  const [tempPrice, setTempPrice] = useState(0)
  const [tempMembershipRequired, setTempMembershipRequired] = useState<string[]>([])
  ...
  const fetchMemberships = useCallback(async () => {
    ...
    try {
      ...
      showToast(error.message || "Không thể tải danh sách membership.", ToastType.Error)
    } finally {
  ...
  const fetchCourseData = useCallback(async () => {
    ...
    setError(null)
    try {
      const response: CourseItem = await CourseService.getCourseById(courseId)
      const transformed: Course = {
        ...
        subCategories: response.subCategories?.map((sc: SubCategory) => sc.id) ?? [],
        status: response.status === 1 ? "public" : "hidden",
        createdAt: response.chapters?.[0]?.createAt ?? new Date().toISOString().split("T")[0],
        chapters:
        ...
        enrollments: 0,
        thumbnail: response.thumble ?? "/placeholder.svg?height=200&width=300",
        thumbnailFile: null,
  ...
  const handleSaveBasicInfo = async () => {
    ...
    try {
      ...
      if (response.success) {
        ...
        showToast("Thông tin khóa học đã được cập nhật.", ToastType.Success)
      } else {
        throw new Error(response.error || "Lỗi khi cập nhật thông tin khóa học.")
      }
    } catch (err: any) {
      showToast(err.message || "Lỗi khi cập nhật thông tin khóa học.", ToastType.Error)
    } finally {
      setIsLoading(false)
    }
  }
  ...
  const handleSaveCategories = async () => {
    ...
    setIsLoading(true)
    try {
      const addedSubCategories = tempSubCategories.filter((id) => !course.subCategories.includes(id))
      await Promise.all(
      ...
      setCourse((prev) => (prev ? { ...prev, subCategories: tempSubCategories } : prev))
  ...
  const handleSaveContent = async () => {
    ...
    setCourse({
      ...course,
      chapters: [...tempChapters].sort((a, b) => new Date(a.createdAt).getTime() - new Date(b.createdAt).getTime()),
    })
  ...
  const handleToggleStatus = async (newStatus: "public" | "hidden") => {
    ...
    if (newStatus === "public" && course.chapters.length === 0) {
      showToast(
    ...
    try {
      ...
      if (response.success) {
        ...
        showToast(`Khóa học đã được ${newStatus === "public" ? "công khai" : "ẩn"}.`, ToastType.Success)
      } else {
  ...
  const handleMembershipChange = (membershipId: string, checked: boolean) => {
    setTempMembershipRequired((prev) => (checked ? [...prev, membershipId] : prev.filter((id) => id !== membershipId)))
  }
  ...
  const handleSubCategoryChange = (subCategoryId: string, checked: boolean) => {
    setTempSubCategories((prev) => (checked ? [...prev, subCategoryId] : prev.filter((id) => id !== subCategoryId)))
  }
