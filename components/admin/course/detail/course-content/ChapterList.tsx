    ...
    <div className="space-y-4">
      {chapters.map((chapter, index) => {
        ...
        if (isCurrentlyEditing && currentEditingChapter) {
          return (
            <div key={chapter.id} className="border rounded-lg bg-blue-50">
              <div className="p-3 bg-blue-100 border-b">
                <div className="flex items-center gap-2">
                  <div className="flex-shrink-0">{getChapterIcon(chapter.type)}</div>
                  <h4 className="font-medium text-blue-800">
                    🔧 Đang chỉnh sửa: Chương {index + 1} - {chapter.title}
                  </h4>
                </div>
              </div>
              <div className="p-4">
        ...
        // Dùng cùng một schema cho QuizBuilder: answers có {id, text, score}
        let questions: any[] = [];

        if (detail?.quiz?.questions && Array.isArray(detail.quiz.questions)) {
          questions = detail.quiz.questions.map((q: any) => ({
            id: q.id,
            question: q.description,
            answers: (q.answers || []).map((a: any, index: number) => ({
              id: a.id ?? `answer_${index}`,
              text: a.text,
              // lấy score gốc từ backend; fallback nếu dữ liệu cũ là points/isCorrect
              score:
          ...
          // dữ liệu local có thể là points hoặc score, chuẩn hóa về score
          questions = (chapter.quiz as any[]).map((q: any) => ({
            ...q,
        ...
        return (
          <div key={chapter.id} className="border rounded-lg">
            <div className="flex items-start gap-4 p-4">
              <div className="flex-shrink-0 mt-1">{getChapterIcon(chapter.type)}</div>
              <div className="flex-1 space-y-2">
                  ...
                  <div className="flex items-center gap-2">
                    {!isCurrentlyEditing && (
                      <Button
                        variant="ghost"
                        size="sm"
                        onClick={() => handleToggleExpansion(chapter.id)}
                        className="p-1"
                      >
                    ...
                    {isEditingMode && !isCurrentlyEditing && (
                      <div className="flex gap-1">
                        <Button variant="ghost" size="sm" onClick={() => onEditChapter(chapter)}>
                ...
                {expandedChapters.has(chapter.id) && !isCurrentlyEditing && (
                  <div className="mt-4 space-y-4 border-t pt-4">
                    {loadingChapters.has(chapter.id) && (
                      <div className="text-center text-muted-foreground">Đang tải chi tiết chương...</div>
                    )}
                    ...
                    {!loadingChapters.has(chapter.id) && !chapterErrors.has(chapter.id) && (
                      <>
                        ...
                        {(chapterType === 'quiz' || chapterDetailType === 'quiz') && (
                          <div className="space-y-4">
                            <Label className="text-sm font-medium">Danh sách câu hỏi:</Label>
                            ...
                            {isEditingMode ? (
                              <QuizBuilder
                                ...
                                onDeleteQuestion={async (questionId: string) => {
                                  if (onDeleteQuestion) return onDeleteQuestion(questionId, chapter.id);
                                  return false;
                                }}
                                onCreateQuestions={async (questions: any[]) => {
                                  if (onCreateQuestions) return onCreateQuestions(chapter.id, questions);
                                  return false;
                                }}
                                isEditMode={true}
                                showQuizInfo={false}
                                ...
                                {questions.length === 0 ? (
                                  <div className="text-center py-8 bg-muted/20 rounded-lg border-2 border-dashed border-muted">
                                    ...
                                    <p className="text-muted-foreground">
                                      Quiz này chưa có câu hỏi. Hãy thêm câu hỏi đầu tiên.
                                    </p>
                                  </div>
                                ) : (
                                  questions.map((question, qIndex) => (
                                    <div key={question.id} className="bg-muted/30 rounded border">
                                            ...
                                            {question.answers.map(
                                              (
                                                ...
                                                >
                                                  ...
                                                  <span
                                                    className={(answer.score ?? 0) > 0 ? 'text-green-700 font-medium flex-1' : 'flex-1'}
                                                  >
                                                    {answer.text}
                                                  </span>
