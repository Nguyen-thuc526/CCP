        ...
        <div className="flex items-center gap-2">
          ...
          {props.isEditingMode ? (
            <div className="flex items-center space-x-2">
              <Switch id="edit-is-free" checked={props.tempIsFree} onCheckedChange={props.setTempIsFree} />
              <Label htmlFor="edit-is-free">Khóa học miễn phí</Label>
            </div>
          ) : props.course.isFree ? (
            <span className="font-semibold text-green-600">Miễn phí</span>
          ) : (
            <span className="font-semibold">{props.course.price.toLocaleString("vi-VN")} VNĐ</span>
          )}
        </div>
              ...
              {props.membershipOptions.map((option) => (
                <div key={option.id} className="flex items-center space-x-2">
                  <Checkbox
                    ...
                    checked={props.tempMembershipRequired.includes(option.id)}
                    onCheckedChange={(checked) => props.onMembershipChange(option.id, checked as boolean)}
                  />
              ...
              {props.course.membershipRequired.length > 0 ? (
                props.course.membershipRequired.map((membership) => (
                  <Badge key={membership} variant="outline">
                    {props.membershipOptions.find((opt) => opt.id === membership)?.label || membership}
                  </Badge>
                ))
              ) : (
