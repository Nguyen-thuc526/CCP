...
export function CourseCategories(props: CourseCategoriesProps) {
  ...
  const handleEdit = () => {
    ...
    props.categories.forEach((category) => {
      const hasSelectedSub = category.subCategories.some((sub) => props.tempSubCategories.includes(sub.id))
      if (hasSelectedSub) {
                  ...
                  {props.categories.map((category) => (
                    ...
                    >
                      <CollapsibleTrigger asChild>
                        <Button variant="ghost" className="w-full justify-start p-2 h-auto font-medium">
                        ...
                        {category.subCategories.map((subCategory) => (
                          <div key={subCategory.id} className="flex items-center space-x-2">
                            <Checkbox
                              ...
                              checked={props.tempSubCategories.includes(subCategory.id)}
                              onCheckedChange={(checked) =>
                                props.onSubCategoryChange(subCategory.id, checked as boolean)
                              }
                            />
