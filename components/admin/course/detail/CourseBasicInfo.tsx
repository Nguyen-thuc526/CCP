...
export function CourseBasicInfo(props: CourseBasicInfoProps) {
  ...
  const priceError = props.isEditingMode ? validatePrice(props.tempPrice) : null;
  ...
  return (
    <div className="space-y-6">
      ...
      <Card className="overflow-hidden">
        <CardHeader className="bg-gray-50 border-b">
          <CardTitle className="text-xl font-bold text-gray-800">Thông tin giá & Yêu cầu</CardTitle>
        </CardHeader>
        <CardContent className="p-6 space-y-6">
          {/* Price */}
          <div className="space-y-3">
            ...
            {props.isEditingMode ? (
              <div className="relative">
                <Input
                  ...
                  className={`pr-12 text-lg font-medium focus:border-gray-500 focus:ring-gray-500 ${
                    priceError ? 'border-red-500 focus:border-red-500 focus:ring-red-500' : 'border-gray-300'
                  }`}
                />
              ...
              <div className="p-4 bg-gray-50 rounded-lg border">
                <span className="text-2xl font-bold text-gray-800">{formatPrice(props.course.price)}</span>
              </div>
            )}
          </div>
          ...
          <div className="space-y-3">
            ...
            {props.isEditingMode ? (
              <Select
                value={props.tempRank === null ? 'null' : String(props.tempRank)}                 
                onValueChange={(value) => props.onRankChange(value === 'null' ? null : Number(value))}  
              >
                ...
                <SelectContent>
                  ...
                  {props.memberships
                    .filter((m) => m.status === 1)
                    .map((membership) => (
                      <SelectItem key={membership.id} value={String(membership.rank)}>
                        <div className="flex items-center gap-2">
