...
export default function QuizBuilder({
  ...
  const [newAnswers, setNewAnswers] = useState<Pick<Answer, 'text' | 'score'>[]>([
    { text: '', score: 0 },
  ]);
  ...
  const handleAddAnswer = () => {
    setNewAnswers([...newAnswers, { text: '', score: 0 }]);
  };
  ...
  const handleAnswerChange = (
    ...
    if (field === 'text') updatedAnswers[index].text = value as string;
    else updatedAnswers[index].score = Number(value);
  ...
  const handleEditQuestion = (question: Question) => {
    // Chuẩn hóa: nếu data cũ dùng points -> map sang score
    ...
    setEditingQuestion({
      ...
      answers: question.answers.map((a: any) => ({
        id: a.id,
        text: a.text,
        score: typeof a.score === 'number' ? a.score : Number(a.points) || 0,
      })),
    });
  };
  ...
  const handleSaveEditQuestion = async () => {
    ...
    if (
      ...
      editingQuestion.answers.some((a) => !a.text.trim())
    ) {
      alert('Vui lòng điền đầy đủ câu hỏi và các lựa chọn trả lời.');
      return;
    }
    ...
    if (isEditMode && onUpdateQuestion) {
      ...
      setLocalQuiz(
        localQuiz.map((q) => (q.id === editingQuestion.id ? editingQuestion : q))
      );
      setEditingQuestionId(null);
      setEditingQuestion(null);
    }
  ...
  const updateEditingQuestion = (updates: Partial<Question>) => {
    if (editingQuestion) setEditingQuestion({ ...editingQuestion, ...updates });
  };

  const updateEditingAnswer = (
    index: number,
    field: 'text' | 'score',
    value: string | number
  ) => {
            ...
            {newAnswers.map((answer, index) => (
              <div key={index} className="flex items-center gap-2">
                ...
                <div className="flex items-center space-x-2 min-w-[140px]">
                  <Label className="text-sm">Điểm (score):</Label>
                  <Input
                    type="number"
                    value={answer.score}
                    onChange={(e) =>
            ...
            <div className="mt-4 space-y-2 border-t pt-4">
              <div className="flex items-center justify-between">
                <Label className="font-medium">
                  Danh sách câu hỏi đã thêm ({pendingQuestions.length})
                </Label>
                <Button onClick={handleSaveQuestions} disabled={isSaving} className="bg-green-600 hover:bg-green-700">
                      ...
                      <div className="flex-1">
                        ...
                        <div className="mt-2 space-y-1">
                          {q.answers.map((a, aIndex) => (
                            <div key={a.id} className="text-xs p-1 rounded bg-gray-100 flex justify-between items-center">
                              <span>{String.fromCharCode(65 + aIndex)}. {a.text}</span>
              ...
              {editingQuestionId === q.id && editingQuestion ? (
                      ...
                      <div key={answer.id} className="flex items-center gap-2">
                        ...
                        <div className="flex items-center space-x-2 min-w-[140px]">
                          ...
                          <Input
                            type="number"
                            value={answer.score}
                            onChange={(e) => updateEditingAnswer(index, 'score', e.target.value)}
                            className="w-20 text-center"
                            placeholder="0,1,2…"
                          />
                        </div>
                ...
                <div>
                  <div className="flex items-center justify-between mb-3">
                    <h4 className="font-medium">Câu hỏi {qIndex + 1}: {q.question}</h4>
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="space-y-1 ml-2">
                      {q.answers.map((a: any, aIndex: number) => {
                        const score = typeof a.score === 'number' ? a.score : Number(a.points) || 0;
                        return (
                          <div
                            key={a.id}
                            className="flex items-center justify-between gap-2 text-sm p-2 rounded bg-gray-50 border border-gray-200"
                          >
