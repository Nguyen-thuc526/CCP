...
export default function QuizBuilder({
   ...
   const [newAnswers, setNewAnswers] = useState([{ text: '', points: 1 }]);
   ...
   const handleAddAnswer = () => {
      setNewAnswers([...newAnswers, { text: '', points: 1 }]);
   };
   ...
   const handleAnswerChange = (
      ...
      if (field === 'text') {
         ...
         updatedAnswers[index].points = value as number;
      }
   ...
   const handleSaveQuestions = async () => {
      ...
      try {
         if (onCreateQuestions) {
            ...
            if (success) {
               if (onLoadChapterDetail && chapterId)
                  await onLoadChapterDetail(chapterId);
               setPendingQuestions([]);
               setShowAddForm(false);
               setNewQuestionText('');
               setNewAnswers([{ text: '', points: 1 }]);
   ...
   const handleRemoveQuestion = async (questionId: string) => {
      if (isEditMode && onDeleteQuestion) {
         ...
         if (success && onLoadChapterDetail && chapterId)
            await onLoadChapterDetail(chapterId);
      } else {
                           ...
                           >
                              <div className="flex items-start justify-between">
                                 <div className="flex-1">
                                    ...
                                    <div className="mt-2 space-y-1">
                                       {q.answers.map((a, aIndex) => (
                                          <div
                                             key={a.id}
                                             className="text-xs p-1 rounded bg-gray-100 flex justify-between items-center"
                                          >
                                             <span>
                                                {String.fromCharCode(
                                                   65 + aIndex
                                 ...
                                 <Button
                                    ...
                                    size="sm"
                                    onClick={() =>
                                       handleRemovePendingQuestion(q.id)
                                    }
         ...
         {localQuiz.length > 0 ? (
            ...
            <div className="text-center py-8 bg-muted/20 rounded-lg border-2 border-dashed border-muted">
               ...
               <p className="text-muted-foreground mb-4">
                  Hãy thêm câu hỏi đầu tiên cho quiz này.
               </p>
               {!showAddForm && (
