...
import { Eye, MoreHorizontal, ToggleLeft, ToggleRight, PlusCircle } from "lucide-react"
...
export function AdminCourseList() {
  ...
  const handleToggleStatus = async (courseId: string, currentStatus: "public" | "hidden") => {
    try {
      if (currentStatus === "hidden") {
        const course = courses.find((c) => c.id === courseId)
        if (course && course.chapters === 0) {
          showToast("Không thể công khai khóa học chưa có chương nào.", ToastType.Error)
          return
        }
      }
      ...
      setCourses((prevCourses) =>
        prevCourses.map((course) =>
          course.id === courseId ? { ...course, status: newStatus === 1 ? "public" : "hidden" } : course,
        ),
      )
      showToast(`Khóa học ${courseId} đã được ${newStatus === 1 ? "công khai" : "ẩn"}.`, ToastType.Success)
    } catch (error) {
        ...
        <Dialog open={isCreateCourseDialogOpen} onOpenChange={setIsCreateCourseDialogOpen}>
          ...
          <DialogContent className="sm:max-w-[425px]">
            <DialogHeader>
              ...
              <DialogDescription>Nhập tên khóa học để bắt đầu.</DialogDescription>
            </DialogHeader>
            ...
            >
              ...
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <Badge className="ml-2" variant={course.status === "public" ? "default" : "secondary"}>
                ...
                <DropdownMenu>
                  ...
                  <DropdownMenuContent align="end">
                    ...
                    <DropdownMenuItem onClick={() => handleToggleStatus(course.id, course.status)}>
                      ...
                      {course.status === "public" ? "Ẩn khóa học" : "Công khai khóa học"}
                    </DropdownMenuItem>
                  </DropdownMenuContent>
                </DropdownMenu>
              </div>
