...
import { loginSchema, registerSchema, RegisterFormData, LoginFormData } from "@/lib/validationSchemas";
...
export function LoginForm() {
  ...
  const handleLogin = async (values: LoginFormData, setSubmitting: (isSubmitting: boolean) => void) => {
    try {
      let response;
  ...
  const handleRegister = async (values: RegisterFormData, setSubmitting: (isSubmitting: boolean) => void) => {
    try {
      ...
      showToast("Đăng ký thất bại. Vui lòng kiểm tra thông tin.", ToastType.Error);
      setSubmitting(false);
    }
  };
  ...
  return (
    <Card className="w-full max-w-md shadow-xl rounded-2xl border">
      ...
      <CardContent>
        <Tabs value={role.toString()} onValueChange={(value) => setFormType(parseInt(value) as Role)} className="space-y-4">
          <TabsList className="grid w-full grid-cols-2 mb-4">
            <TabsTrigger value={Role.Admin.toString()}>Quản trị viên</TabsTrigger>
            <TabsTrigger value={Role.Counselor.toString()}>Tư vấn viên</TabsTrigger>
          </TabsList>
            ...
            {isRegistering ? (
              ...
              >
                {({ isSubmitting }) => (
                  <Form className="space-y-4 pt-4">
                    <div className="space-y-2">
                      <Label htmlFor="counselor-fullName">Họ tên</Label>
                      <Field
                        as={Input}
                    ...
                    <div className="space-y-2">
                      <Label htmlFor="counselor-confirmPassword">Xác nhận mật khẩu</Label>
                      <Field
                        as={Input}
