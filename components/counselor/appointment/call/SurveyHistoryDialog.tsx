...
export function SurveyHistoryDialog({
         ...
         try {
            if (bookingType === 'individual') {
               ...
               for (const surveyId of SURVEY_IDS) {
                  try {
                     const response =
                        await bookingService.personTypeBeforeBooking({
                           memberId,
                           surveyId,
                           bookingId,
                        });
   ...
   const handleSendCompleteHistory = async (
      recipientId: string,
      ...
      try {
         ...
         showToast(
            `Đã gửi kết quả khảo sát cho ${recipientName}`,
            ToastType.Success
         );
      } catch (error) {
   ...
   const handleSendCoupleToIndividual = async (
      recipientId: string,
      recipientName: string
   ) => {
   ...
   const handleSendToBoth = async () => {
      ...
      try {
         ...
         showToast(
            'Đã gửi kết quả khảo sát cho cả hai người',
            ToastType.Success
         );
      } catch (error) {
         showToast('Có lỗi xảy ra khi gửi kết quả khảo sát', ToastType.Error);
   ...
   const handleSendCoupleAnalysis = async () => {
      ...
      try {
         ...
         showToast(
            'Đã gửi phân tích cặp đôi cho cả hai người',
            ToastType.Success
         );
      } catch (error) {
               ...
               {memberData.mbti && (
                  <div className="p-3 bg-blue-50 rounded-lg">
                     ...
                     {Object.keys(memberData.mbtiScores).length > 0 &&
                        renderScoreChart(memberData.mbtiScores)}
                  </div>
               )}
            ...
            <div className="flex items-center justify-between">
               ...
               <div className="flex gap-2">
                  <Button
                     ...
                     disabled={sendingStates['couple-both']}
                     size="sm"
                     variant="outline"
                  >
                  ...
                  <Button
                     onClick={handleSendCoupleAnalysis}
                     disabled={sendingStates['couple-analysis']}
                     size="sm"
                  >
            ...
            <DialogHeader>
               <DialogTitle className="flex items-center gap-2 text-base sm:text-lg">
                  ...
                  <span className="text-balance">
                     Lịch sử khảo sát{' '}
                     {bookingType === 'individual' ? 'cá nhân' : 'cặp đôi'}
                  </span>
               </DialogTitle>
            </DialogHeader>
               ...
               {loading ? (
                  ...
                  <div className="space-y-4">
                     <div className="flex items-center gap-2 mb-4">
                        ...
                        <h3 className="text-base sm:text-lg font-semibold text-balance">
                           Kết quả mới nhất - {memberName}
                        </h3>
                     </div>
                        ...
                        <>
                           ...
                           <div className="flex justify-center pt-4 border-t">
                              ...
                              >
                                 ...
                                 <span className="text-balance">
                                    Gửi kết quả khảo sát cho {memberName}
                                 </span>
                              </Button>
                           </div>
                        </>
                  ...
                  <div className="space-y-4">
                     ...
                     {!coupleData ? (
                        <div className="text-center py-8">
                           <Info className="h-8 w-8 mx-auto mb-2 text-gray-400" />
                           <div className="text-sm text-gray-500">
                              Chưa có kết quả khảo sát cặp đôi
                           </div>
                        </div>
                     ) : (
                        renderCoupleSurvey()
