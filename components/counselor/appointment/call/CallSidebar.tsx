...
export default function CallSidebar({
         ...
         if (bookingInfo.type === 'individual') {
            ...
            for (const surveyId of SURVEY_IDS) {
               try {
                  ...
                  results[surveyId] = response?.data
                     ? (Array.isArray(response.data) ? response.data : [response.data])
                        ...
                        .sort((a: SurveyResult, b: SurveyResult) => new Date(b.createAt).getTime() - new Date(a.createAt).getTime())
                     : [];
               } catch {
                  results[surveyId] = [];
               }
            ...
            const results: Record<string, SurveyResult[]> = {};
            for (const surveyId of SURVEY_IDS) {
               results[`${surveyId}_member`] = [];
               if (bookingInfo.partnerId) {
                  results[`${surveyId}_partner`] = [];
               }
               ...
               try {
                  ...
                  if (memberResult?.data) {
                     results[`${surveyId}_member`] = (Array.isArray(memberResult.data) ? memberResult.data : [memberResult.data])
                        .map((item: any) => ({
                           surveyId,
                           result: item.result || item.type || '',
                           description: item.description || '',
                  ...
                  if (bookingInfo.partnerId && partnerResult?.data) {
                     results[`${surveyId}_partner`] = (Array.isArray(partnerResult.data) ? partnerResult.data : [partnerResult.data])
                        .map((item: any) => ({
                           surveyId,
                           result: item.result || item.type || '',
                           description: item.description || '',
            ...
            try {
               ...
               // No couple data, rely on individual results
            }
         }
      } catch (error) {
   ...
   const renderIndividualSurvey = (surveyId: string, results: SurveyResult[], memberName: string) => {
   ...
   const renderCompatibilityCard = (data: CoupleData) => {
      const blocks = [
         ...
         { key: 'loveLanguage', title: 'Love Language', icon: Heart, color: 'pink', detail: data.loveLanguageDetail },
         ...
         <div className="space-y-4">
            {blocks.map(({ key, title, icon: Icon, color, detail }: any) => (
               <Card key={key} className="border">
                  ...
                  <CardContent className="space-y-3">
                     <div className={`flex items-center justify-between p-3 bg-${color}-50 rounded-lg border border-${color}-100`}>
                        <div className="flex items-center gap-3">
                           {detail.image && (
                              <img src={detail.image || '/placeholder.svg'} alt="Compatibility" className="w-12 h-12 object-contain" />
                           )}
                           <div>
                     ...
                     {detail.detail && (
                        <details className="group">
                           ...
                           <div className="mt-2 space-y-3">
                              <div className="prose prose-xs max-w-none text-gray-700 text-xs leading-relaxed" dangerouslySetInnerHTML={{ __html: detail.detail }} />
                              <div className="space-y-3">
                                 {detail.strongPoints && (
                                    <div className={`p-2 bg-green-50 rounded border border-green-100`}>
                                       ...
                                       <div className="prose prose-xs max-w-none text-green-800 text-xs" dangerouslySetInnerHTML={{ __html: detail.strongPoints }} />
                                    </div>
                                 )}
                                 {detail.weaknesses && (
                                    <div className="p-2 bg-orange-50 rounded border border-orange-100">
                                       ...
                                       <div className="prose prose-xs max-w-none text-orange-800 text-xs" dangerouslySetInnerHTML={{ __html: detail.weaknesses }} />
                                    </div>
                                 )}
                              </div>
                           </div>
   ...
   const renderCoupleSurvey = () => {
      ...
      if (!coupleData && bookingInfo?.type === 'couple') {
         // Check for any valid survey results for member or partner
         const hasMemberResults = SURVEY_IDS.some(
            (surveyId) => Array.isArray(individualResults[`${surveyId}_member`]) && individualResults[`${surveyId}_member`].length > 0
         );
         const hasPartnerResults = bookingInfo.partnerId && SURVEY_IDS.some(
            (surveyId) => Array.isArray(individualResults[`${surveyId}_partner`]) && individualResults[`${surveyId}_partner`].length > 0
         );

         if (!hasMemberResults && !hasPartnerResults) {
            return (
               <div className="text-center py-10">
                  <Info className="h-8 w-8 mx-auto mb-2 text-gray-400" />
                  <div className="text-sm text-gray-500">Chưa có thông tin khảo sát cho cả hai thành viên.</div>
               </div>
            );
         }

         return (
            <Tabs defaultValue={hasMemberResults ? 'member' : 'partner'}>
               ...
               <TabsContent value="member" className="space-y-4">
                  <div className="flex items-center gap-2 mb-4">
                     ...
                     <h3 className="text-base font-semibold">Kết quả khảo sát - {bookingInfo.memberName}</h3>
                  </div>
                  {hasMemberResults ? (
                     <div className="grid grid-cols-1 gap-4">
                        {SURVEY_IDS.map((surveyId) =>
                           renderIndividualSurvey(surveyId, individualResults[`${surveyId}_member`] || [], bookingInfo.memberName)
                        ).filter(Boolean)}
                     </div>
                  ) : (
                     <div className="text-center py-8">
                        <Info className="h-8 w-8 mx-auto mb-2 text-gray-400" />
                        <div className="text-sm text-gray-500">Chưa có kết quả khảo sát cho {bookingInfo.memberName}.</div>
                     </div>
                  )}
               </TabsContent>
               {bookingInfo.partnerName && (
                  <TabsContent value="partner" className="space-y-4">
                     <div className="flex items-center gap-2 mb-4">
                        <Star className="h-4 w-4 text-yellow-600" />
                        <h3 className="text-base font-semibold">Kết quả khảo sát - {bookingInfo.partnerName}</h3>
                     </div>
                     {hasPartnerResults ? (
                        <div className="grid grid-cols-1 gap-4">
                           {SURVEY_IDS.map((surveyId) =>
                              renderIndividualSurvey(surveyId, individualResults[`${surveyId}_partner`] || [], bookingInfo.partnerName || '')
                           ).filter(Boolean)}
                        </div>
                     ) : (
                        <div className="text-center py-8">
                           <Info className="h-8 w-8 mx-auto mb-2 text-gray-400" />
                           <div className="text-sm text-gray-500">Chưa có kết quả khảo sát cho {bookingInfo.partnerName}.</div>
                        </div>
                     )}
                  </TabsContent>
               )}
      ...
      if (!coupleData) {
         return (
            <div className="text-center py-10">
               <Info className="h-8 w-8 mx-auto mb-2 text-gray-400" />
               <div className="text-sm text-gray-500">Chưa có thông tin khảo sát.</div>
            </div>
         );
      }
   ...
   return (
      <div className={`fixed top-0 right-0 h-full bg-white border-l shadow-lg transition-all duration-300 z-50 ${isOpen ? 'w-96' : 'w-0'} overflow-hidden`}>
         <Button
            onClick={onToggle}
            variant="outline"
            size="sm"
         ...
         <Tabs defaultValue="notes" className="h-full flex flex-col">
            ...
            <TabsContent value="notes" className="flex-1 overflow-y-auto p-4 m-0">
               <div className="space-y-4">
                  <Card>
                     <CardHeader className="pb-3 cursor-pointer hover:bg-gray-50 transition-colors" onClick={() => toggleSection('problemSummary')}>
                        <CardTitle className="text-sm flex items-center gap-2">
                           <FileText className="h-4 w-4 text-blue-600" />
                           Tóm tắt vấn đề
                           <Badge variant="destructive" className="ml-2 text-xs">Bắt buộc</Badge>
                           {collapsedSections.problemSummary ? <ChevronDown className="h-4 w-4 ml-auto" /> : <ChevronUp className="h-4 w-4 ml-auto" />}
                        </CardTitle>
                     </CardHeader>
                  ...
                  <Card>
                     <CardHeader className="pb-3 cursor-pointer hover:bg-gray-50 transition-colors" onClick={() => toggleSection('problemAnalysis')}>
                        <CardTitle className="text-sm flex items-center gap-2">
                           <Search className="h-4 w-4 text-green-600" />
                           Phân tích vấn đề
                           <Badge variant="outline" className="ml-2 text-xs">Tùy chọn</Badge>
                           {collapsedSections.problemAnalysis ? <ChevronDown className="h-4 w-4 ml-auto" /> : <ChevronUp className="h-4 w-4 ml-auto" />}
                        </CardTitle>
                     </CardHeader>
                  ...
                  <Card>
                     <CardHeader className="pb-3 cursor-pointer hover:bg-gray-50 transition-colors" onClick={() => toggleSection('guides')}>
                        <CardTitle className="text-sm flex items-center gap-2">
                           <Lightbulb className="h-4 w-4 text-amber-600" />
                           Hướng dẫn
                           <Badge variant="destructive" className="ml-2 text-xs">Bắt buộc</Badge>
                           {collapsedSections.guides ? <ChevronDown className="h-4 w-4 ml-auto" /> : <ChevronUp className="h-4 w-4 ml-auto" />}
                        </CardTitle>
                     </CardHeader>
               ...
               <div className="space-y-4">
                  {loadingSurvey ? (
                     ...
                     <div className="space-y-4">
                        ...
                        {Object.keys(individualResults).length === 0 || !SURVEY_IDS.some((surveyId) => individualResults[surveyId]?.length > 0) ? (
                           <div className="text-center py-8">
                              <Info className="h-8 w-8 mx-auto mb-2 text-gray-400" />
                           ...
                           <div className="grid grid-cols-1 gap-4">
                              {SURVEY_IDS.map((surveyId) => renderIndividualSurvey(surveyId, individualResults[surveyId] || [], bookingInfo.memberName)).filter(
                                 Boolean
                              )}
                           </div>
                        )}
