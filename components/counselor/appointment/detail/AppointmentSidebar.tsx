    ...
    <div className="space-y-6">
      ...
      <Card>
        ...
        <CardContent className="space-y-3">
          ...
          {status === BookingStatus.Finish && effectiveStatus === BookingStatus.Finish && (
            <>
              <div className="bg-amber-50 border border-amber-200 rounded-lg p-4">
                <div className="flex items-center gap-2 text-amber-700 mb-2">
                  <Clock className="h-5 w-5" />
                  <span className="font-medium">Tự động chuyển tiền sau:</span>
                </div>
                {!autoCompleteCountdown.isExpired ? (
                  <div className="text-center">
                    <div className="text-2xl font-mono font-bold text-amber-800 mb-1">
                      {autoCompleteCountdown.hours.toString().padStart(2, "0")}:
                      {autoCompleteCountdown.minutes.toString().padStart(2, "0")}:
                      {autoCompleteCountdown.seconds.toString().padStart(2, "0")}
                    </div>
                    <p className="text-xs text-amber-600">Sau thời gian này tiền sẽ chuyển về ví của bạn</p>
                  </div>
                ) : (
                  <p className="text-sm text-amber-600 text-center">Đang xử lý chuyển tiền...</p>
                )}
              </div>
              <Button variant="outline" onClick={onOpenNoteDialog} className="w-full bg-transparent">
                <MessageSquare className="mr-2 h-4 w-4" />
          ...
          {effectiveStatus === BookingStatus.Complete && (
            <>
              <div className="bg-green-50 border border-green-200 rounded-lg p-4">
                <div className="flex items-center gap-2 text-green-700">
                  <CheckCircle className="h-5 w-5" />
                  <span className="font-medium">Đã chuyển tiền và kết thúc</span>
                </div>
              </div>
              <Button variant="outline" onClick={onOpenNoteDialog} className="w-full bg-transparent">
                <MessageSquare className="mr-2 h-4 w-4" />
