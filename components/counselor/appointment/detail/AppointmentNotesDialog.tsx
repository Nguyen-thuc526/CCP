...
import { FileText, Search, Lightbulb, User, Calendar, Clock, Edit3, Save, X } from "lucide-react"
...
const AppointmentNotesDialog: React.FC<AppointmentNotesDialogProps> = ({
  ...
  const memberName = appointment.member2?.fullname
      ...
      <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
        <DialogHeader className="pb-4">
          <div className="flex items-center justify-between">
            ...
            {!isEditing &&
              (appointment.status === BookingStatus.Finish || appointment.status === BookingStatus.Complete) && (
                <Button onClick={() => setIsEditing(true)} size="sm" className="gap-2">
                  <Edit3 className="h-4 w-4" />
                  {hasNotes ? "Chỉnh sửa" : "Thêm ghi chú"}
                </Button>
              )}
          </div>
        </DialogHeader>
          ...
          <Card>
            <CardContent className="p-8 text-center">
              <div className="flex flex-col items-center gap-3">
                ...
                <div>
                  <p className="text-gray-900 font-medium">Chưa có ghi chú tư vấn</p>
                  <p className="text-gray-500 text-sm">Nhấn "Thêm ghi chú" để thêm ghi chú cho buổi tư vấn này.</p>
                </div>
              </div>
            </CardContent>
          </Card>
          ...
          {isEditing &&
            (appointment.status === BookingStatus.Finish || appointment.status === BookingStatus.Complete) && (
              <Button onClick={handleSave} className="gap-2" disabled={loading}>
                ...
                {hasNotes ? "Lưu ghi chú" : "Thêm ghi chú"}
              </Button>
            )}
        </div>
      </DialogContent>
