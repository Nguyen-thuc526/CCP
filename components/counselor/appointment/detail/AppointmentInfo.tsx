...
const AppointmentInfo: React.FC<AppointmentInfoProps> = ({
  ...
  const duration = Math.floor((new Date(timeEnd).getTime() - startDate.getTime()) / (1000 * 60))
    ...
    <Card>
      ...
      <CardContent className="space-y-4">
        ...
        <div>
          ...
          <p className="mt-1 font-medium">{subCategories.map((sub) => sub.name).join(", ") || "Không xác định"}</p>
        </div>

        {(status === BookingStatus.Complete || status === BookingStatus.Finish) && hasNotes && (
          <div className="p-4 bg-green-50 border border-green-200 rounded-lg space-y-3">
            <Label className="text-sm font-medium text-green-800">Ghi chú sau buổi tư vấn</Label>

            {problemSummary && (
              <div>
                <Label className="text-xs font-medium text-green-700">Tóm tắt vấn đề:</Label>
                <p className="mt-1 text-sm text-green-700">{problemSummary}</p>
              </div>
            )}

            {problemAnalysis && (
              <div>
                <Label className="text-xs font-medium text-green-700">Phân tích vấn đề:</Label>
                <p className="mt-1 text-sm text-green-700">{problemAnalysis}</p>
              </div>
            )}

            {guides && (
              <div>
                <Label className="text-xs font-medium text-green-700">Hướng dẫn:</Label>
                <p className="mt-1 text-sm text-green-700">{guides}</p>
              </div>
            )}
          </div>
        )}
        ...
        {status === BookingStatus.Refund && feedback && (
          <div className="p-4 bg-red-50 border border-red-200 rounded-lg">
            <Label className="text-sm font-medium text-red-800">Phản hồi từ khách hàng (Lý do hoàn tiền)</Label>
        ...
        {note && (
          <div className="p-4 bg-blue-50 border border-blue-200 rounded-lg">
            <Label className="text-sm font-medium text-blue-800">Ghi chú từ khách hàng</Label>
