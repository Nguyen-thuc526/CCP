...
function normalizePersonTypeResponses(
   surveyId: SurveyId,
   apiData: any
): SurveyResult[] {
   if (!apiData) return [];
   ...
   return arr.map((item) => {
      ...
      const scores: Record<string, number> =
         (typeof scoresFromObj === 'object' && scoresFromObj) ||
         parseRawScores(meta.rawScores || meta.RawScores) ||
         {};
...
function calculateMBTIPercentages(
   scores: Record<string, number>
...
export function IndividualSurveyResults({
   memberName,
   memberId,
   bookingId,
}: IndividualSurveyResultsProps) {
   const [loading, setLoading] = useState(false);
   const [error, setError] = useState<string | null>(null);
   const [resultsBySurvey, setResultsBySurvey] = useState<
      Partial<Record<SurveyId, SurveyResult[]>>
   >({});
   ...
   const handleExportPDF = async () => {
      ...
      try {
         ...
         pdf.save(
            `ket-qua-khao-sat-${memberName.replace(/\s+/g, '-').toLowerCase()}.pdf`
         );
      } catch (e) {
         console.error(e);
      } finally {
   ...
   const resultsByDate = useMemo(() => {
      ...
      Object.keys(dateGroups).forEach((dateKey) => {
         dateGroups[dateKey].sort((a, b) => {
            const surveyOrder =
               SURVEY_IDS.indexOf(a.surveyId) - SURVEY_IDS.indexOf(b.surveyId);
   ...
   const hasAny = useMemo(
      () =>
         Object.values(resultsBySurvey).some((arr) => (arr?.length ?? 0) > 0),
      [resultsBySurvey]
   );
   ...
   const renderCompactSurveyCard = (result: SurveyResult, showDate = false) => {
      ...
      const mergedDescription =
         result.typeDetail?.description ?? result.description;
                           ...
                           <div key={date}>
                              <div className="flex items-center gap-2 mb-3">
                                 ...
                                 <span className="text-sm text-gray-500">
                                    {results.length} khảo sát
                                 </span>
                              </div>
                              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                                 {results.map((result) =>
                                    renderCompactSurveyCard(result)
                                 )}
                              </div>
                           </div>
                        ))}
         ...
         <div className="flex items-center justify-between gap-4">
            {loading ? (
               <div className="text-sm text-gray-500 flex items-center gap-2">
                  <BrainIcon className="h-4 w-4 animate-pulse" /> Đang tải kết
                  quả...
               </div>
            ) : error ? (
