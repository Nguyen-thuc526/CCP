   ...
   return (
      <div className="space-y-3">
         {mbtiPairs.map((pair) => {
            ...
            const progressValue =
               leftScore > rightScore ? leftPercentage : 100 - rightPercentage;
...
function renderMemberCard(
   ...
   const loveLanguageType = isFirst
      ? coupleData?.loveLanguage
      : coupleData?.loveLanguage1;
   const bigFiveType = isFirst ? coupleData?.bigFive : coupleData?.bigFive1;

   const mbtiScores = parseScores(
      isFirst ? coupleData?.mbtiDescription : coupleData?.mbti1Description
   );
   const discScores = parseScores(
      isFirst ? coupleData?.discDescription : coupleData?.disc1Description
   );
   const loveLanguageScores = parseScores(
      isFirst
         ? coupleData?.loveLanguageDescription
         : coupleData?.loveLanguage1Description
   );
   const bigFiveScores = parseScores(
      isFirst ? coupleData?.bigFiveDescription : coupleData?.bigFive1Description
   );
         ...
         <CardContent className="space-y-4">
            ...
            {mbtiType && (
               <div className="p-3 bg-blue-50 rounded-lg">
                  ...
                  {Object.keys(mbtiScores).length > 0 &&
                     renderMBTIChart(mbtiScores)}
               </div>
            )}
            ...
            {discType && (
               <div className="p-3 bg-green-50 rounded-lg">
                  ...
                  {Object.keys(discScores).length > 0 &&
                     renderScoreChart(discScores)}
               </div>
            )}
               ...
               <div className="p-3 bg-pink-50 rounded-lg">
                  <div className="flex items-center gap-2 mb-2">
                     ...
                     <span className="font-semibold text-pink-900">
                        Love Language: {loveLanguageType}
                     </span>
                  </div>
                  {Object.keys(loveLanguageScores).length > 0 &&
                     renderScoreChart(loveLanguageScores)}
               </div>
            )}
               ...
               <div className="p-3 bg-purple-50 rounded-lg">
                  <div className="flex items-center gap-2 mb-2">
                     ...
                     <span className="font-semibold text-purple-900">
                        Big Five: {bigFiveType}
                     </span>
                  </div>
                  {Object.keys(bigFiveScores).length > 0 &&
                     renderScoreChart(bigFiveScores)}
               </div>
            )}
         </CardContent>
      </Card>
               ...
               <Card key={detail.key}>
                  <CardHeader>
                     <CardTitle className="text-lg flex items-center gap-2">
                        <IconComponent
                           className={`h-5 w-5 text-${detail.color}-600`}
                        />
                        {detail.title}
                     </CardTitle>
                  </CardHeader>
                  <CardContent className="space-y-4">
                     ...
                     >
                        <div className="flex items-center gap-3">
                           ...
                           <div>
                              <div
                                 className={`text-2xl font-bold text-${detail.color}-900`}
                              >
                                 {detail.data.compatibility}%
                              </div>
                              <div
                                 className={`text-sm text-${detail.color}-700`}
                              >
                                 Độ tương thích
                              </div>
                           </div>
                        ...
                        <div className="text-right">
                           <div className="font-semibold text-gray-900">
                              {detail.data.personType?.name} &{' '}
                              {detail.data.personType2?.name}
                           </div>
                     ...
                     <Progress
                        value={detail.data.compatibility}
                        className="h-3"
                     />
                     ...
                     {detail.data.detail && (
                        <div className="space-y-4">
                           ...
                           {detail.data.strongPoints && (
                              <div
                                 className={`p-4 bg-${detail.color === 'green' ? 'emerald' : 'green'}-50 rounded-lg`}
                              >
                                 <h4
...
export function CoupleSurveyResults({
   memberName,
   partnerName,
   bookingId,
}: CoupleSurveyResultsProps) {
   ...
   return (
      <div className="space-y-6">
         ...
         <div className="flex items-center gap-4">
            ...
            <UIBadge
               variant="outline"
               className="ml-2 bg-purple-50 border-purple-200"
            >
         ...
         {coupleData && (
            <div className="text-sm text-gray-500">
               Ngày tạo: {formatDate(coupleData.createAt)} • Mã truy cập:{' '}
               {coupleData.accessCode}
            </div>
         )}
         ...
         <div ref={exportRef} className="space-y-6">
            {loading ? (
               <div className="text-center py-8">
                  ...
                  <div className="text-sm text-gray-500">
                     Đang tải kết quả khảo sát cặp đôi...
                  </div>
               </div>
            ) : error ? (
               ...
               <div className="rounded-md border p-4 bg-gray-50 text-gray-600 flex items-start gap-2">
                  ...
                  <div>
                     ...
                     <div className="text-sm">
                        Chưa có kết quả khảo sát cho cặp đôi này.
                     </div>
                  </div>
               </div>
            ) : (
