...
import { CalendarDays, Clock, Users, History, CalendarIcon } from "lucide-react"
...
export default function BookingManagement() {
  ...
  const selectedDateBookings = filteredBookings.filter((booking) => {
    ...
    return new Date(booking.timeStart).toDateString() === selectedDate.toDateString()
  })
  ...
  const paginatedHistoryBookings = filteredHistoryBookings.slice((historyPage - 1) * PAGE_SIZE, historyPage * PAGE_SIZE)
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-4">
            ...
            <div className="lg:col-span-1">
              <Card>
                ...
                <CardContent className="p-3">
                  <CalendarGrid bookings={bookings} selectedDate={selectedDate} onSelectDate={setSelectedDate} />
                  ...
                  {selectedDate && (
                    <div className="mt-4 p-3 bg-blue-50 rounded-lg">
                      ...
                      <div className="flex items-center gap-2 mt-1">
                        ...
                        <span className="text-sm text-blue-700">{selectedDateBookings.length} lịch hẹn</span>
                      </div>
                    </div>
                  )}
                </CardContent>
            ...
            <div className="lg:col-span-3 space-y-4">
                  ...
                  <CardContent>
                    <div className="flex flex-wrap gap-2">
                      ...
                      <Badge variant="outline" className="bg-green-50">
                        Cá nhân: {selectedDateBookings.filter((b) => !b.isCouple).length}
                      </Badge>
                      <Badge variant="outline" className="bg-purple-50">
                        Cặp đôi: {selectedDateBookings.filter((b) => b.isCouple).length}
                      </Badge>
                    </div>
                  </CardContent>
                </Card>
              ...
              {!loading && (
                <div className="text-sm text-muted-foreground">
                  ...
                  {selectedDate && ` cho ngày ${format(selectedDate, "dd/MM/yyyy", { locale: vi })}`}
                </div>
              )}

              <BookingTableUpdated bookings={selectedDateBookings} isLoading={loading} />
        ...
        <TabsContent value="history" className="space-y-4">
          <div className="space-y-4">
            ...
            <Card>
              ...
              <CardContent>
                <div className="flex flex-wrap gap-2">
                  ...
                  <Badge variant="outline" className="bg-green-50">
                    Cá nhân: {filteredHistoryBookings.filter((b) => !b.isCouple).length}
                  </Badge>
                  <Badge variant="outline" className="bg-purple-50">
                    Cặp đôi: {filteredHistoryBookings.filter((b) => b.isCouple).length}
                  </Badge>
                </div>
              </CardContent>
            </Card>
            ...
            {!historyLoading && (
              <div className="text-sm text-muted-foreground">
                Hiển thị {paginatedHistoryBookings.length} trong tổng {filteredHistoryBookings.length} lịch hẹn
              </div>
            )}

            <BookingTableUpdated bookings={paginatedHistoryBookings} isLoading={historyLoading} />
              ...
              <Pagination>
                <PaginationContent>
                  ...
                  {Array.from({
                    ...
                    return (
                      <PaginationItem key={page}>
                        <PaginationLink isActive={historyPage === page} onClick={() => setHistoryPage(page)}>
                          {page}
                        </PaginationLink>
                      </PaginationItem>
                    )
