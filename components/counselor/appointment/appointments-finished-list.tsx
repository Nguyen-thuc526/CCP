...
import { Calendar, Clock, Video, Eye, Users, Filter, Search, FileText, Edit3 } from "lucide-react"
...
export default function AppointmentsFinishedList({
  ...
  const filteredAppointments = useMemo(() => {
    let result = appointments.filter((appointment) => (statusFilter ? appointment.status === statusFilter : true))
  ...
  const openNoteDialog = (appointment: Appointment) => {
    ...
    // Determine if we should start in editing mode
    const hasNotes =
      appointment.notes &&
