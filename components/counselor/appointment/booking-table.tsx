...
export function BookingTableUpdated({ bookings, isLoading = false }: BookingTableProps) {
  const [countdowns, setCountdowns] = useState<
    Record<string, { hours: number; minutes: number; seconds: number; isExpired: boolean }>
  >({})
  ...
  useEffect(() => {
    const calculateCountdowns = () => {
      const newCountdowns: Record<string, { hours: number; minutes: number; seconds: number; isExpired: boolean }> = {}
      ...
      bookings.forEach((booking) => {
        if (booking.status === BookingStatus.Finish) {
          ...
          if (remaining > 0) {
            ...
            newCountdowns[booking.id] = { hours, minutes, seconds, isExpired: false }
          } else {
            newCountdowns[booking.id] = { hours: 0, minutes: 0, seconds: 0, isExpired: true }
          }
        }
      })
  ...
  return (
    <div className="rounded-md border overflow-x-auto">
      <Table>
        <TableHeader>
          <TableRow>
            <TableHead className="whitespace-nowrap">Thành viên</TableHead>
            <TableHead className="whitespace-nowrap">Ngày & Giờ</TableHead>
            <TableHead className="whitespace-nowrap">Thời lượng</TableHead>
        ...
        <TableBody>
          {isLoading ? (
            Array.from({ length: 8 }).map((_, index) => <SkeletonRow key={`skeleton-${index}`} />)
          ) : bookings.length === 0 ? (
            <TableRow>
              <TableCell colSpan={8} className="text-center py-8">
                <div className="flex flex-col items-center gap-2">
                  <div className="text-gray-400 text-sm">Không có dữ liệu booking</div>
                </div>
              </TableCell>
            </TableRow>
          ) : (
                ...
                <TableRow key={booking.id}>
                      ...
                      {booking.isCouple && booking.member2 ? (
                        <>
                          <div className="flex -space-x-2">
                            <Avatar className="h-8 w-8 border-2 border-white shadow-sm">
                              <AvatarImage
                                src={
                                  booking.member.avatar || "/placeholder.svg?height=40&width=40" || "/placeholder.svg"
                                }
                              />
                            ...
                            <Avatar className="h-8 w-8 border-2 border-white shadow-sm">
                              <AvatarImage
                                src={
                                  booking.member2.avatar || "/placeholder.svg?height=40&width=40" || "/placeholder.svg"
                                }
                              />
                        ...
                        <>
                          <Avatar className="h-8 w-8">
                            <AvatarImage
                              src={booking.member.avatar || "/placeholder.svg?height=40&width=40" || "/placeholder.svg"}
                            />
                            <AvatarFallback className="bg-blue-100 text-blue-700 font-semibold">
                              {booking.member.fullname.charAt(0)}
                            </AvatarFallback>
                          </Avatar>
                          <span className="font-medium text-gray-900 text-sm">{booking.member.fullname}</span>
                        </>
                      )}
                    </div>
                  </TableCell>
                  ...
                  <TableCell className="align-middle whitespace-nowrap">
                    <div className="flex flex-col gap-1">
                      <span className="text-sm font-medium text-gray-900">{formatDateTime(booking.timeStart)}</span>
                      <span className="text-xs text-gray-500">đến {format(new Date(booking.timeEnd), "HH:mm")}</span>
                    </div>
                  </TableCell>

                  {/* Thời lượng */}
                  <TableCell className="align-middle whitespace-nowrap">
                    <span className="text-gray-900 text-sm font-medium">{mins} phút</span>
                  </TableCell>
                    ...
                    <div className="max-w-48">
                      {booking.subCategories && booking.subCategories.length > 0 ? (
                        <div className="flex flex-wrap gap-1.5">
                          {booking.subCategories.slice(0, 2).map((category, index) => (
                            <Badge
                              key={category.id}
                              variant="outline"
                  ...
                  <TableCell className="align-middle whitespace-nowrap">
                    <div className="flex flex-col gap-1">
                      <BookingStatusBadge status={booking.status} timeEnd={booking.timeEnd} />
                      {booking.status === BookingStatus.Finish && countdown && !countdown.isExpired && (
                        <Badge
                          variant="outline"
                  ...
                  <TableCell className="align-middle whitespace-nowrap">
                    <div className="flex gap-2">
                      <Button variant="ghost" size="sm" asChild>
                        <Link href={`/counselor/appointments/${booking.id}`}>
                          <Eye className="w-4 h-4" />
                        </Link>
                      </Button>

                      {booking.isCouple ? (
                        <Button variant="ghost" size="sm" asChild title="Xem kết quả khảo sát cặp đôi">
                          <Link
                        ...
                        <Button variant="ghost" size="sm" asChild title="Xem kết quả khảo sát cá nhân">
                          <Link
