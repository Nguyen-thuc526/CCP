  ...
  const fetchWalletData = async () => {
    try {
      ...
      if (
        ...
        // ép kiểu để tránh lệch shape WalletData
        setWalletData({
          balance: walletResponse.data.remaining,
          monthlyEarnings: 8_500_000,
          pendingPayments: 2_250_000,
          totalWithdrawn: 45_000_000,
          availableForWithdrawal: walletResponse.data.remaining
        } as WalletData);
      } else {
        setErrorMessage(
          walletResponse.error || 'Không thể tải dữ liệu ví. Vui lòng thử lại.'
        );
      }
      ...
      if (transactionResponse.success && transactionResponse.data) {
        const completedBookings: Transaction[] =
          transactionResponse.data.items.map(
      ...
      const withdrawApiResponse: WithdrawListResponse =
        await TransactionService.getMyWithdraws();
