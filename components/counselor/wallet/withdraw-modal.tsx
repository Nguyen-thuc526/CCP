...
export function WithdrawModal({ isOpen, onClose, onSuccess }: WithdrawModalProps) {
  ...
  const validateForm = () => {
    ...
    if (!formData.accountName.trim()) {
      ...
      newErrors.accountName = "Tên chủ tài khoản chỉ được chứa chữ cái và khoảng trắng"
    }
          ...
          <Card>
            <CardContent className="pt-6">
              <div className="space-y-2">
                ...
                {formData.total > 0 && (
                  <p className="text-sm text-muted-foreground">{formatCurrency(formData.total)}</p>
                )}
                {errors.total && (
              ...
              <SelectContent>
                {vietnameseBanks.map((bank) => (
                  <SelectItem key={bank.code} value={bank.code} className="h-16">
                    <div className="flex items-center gap-3">
                      <img src={bank.logo || "/placeholder.svg"} alt={bank.name} className="h-8 w-8 rounded" />
                      <div>
