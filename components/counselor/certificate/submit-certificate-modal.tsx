...
import { FilePlus, Loader2, X, ChevronDown, ChevronRight, CheckCircle, Clock, AlertCircle } from "lucide-react"
...
const validateForm = (formData: any, selectedSubCategories: string[], hasImage: boolean): ValidationErrors => {
...
export default function SubmitCertificateModal({ open, onClose, onSuccess }: SubmitCertificateModalProps) {
  ...
  const handleBlur = (field: string) => {
    ...
    const fieldErrors = validateForm(formData, selectedSubCategories, !!formData.image)
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Validate all fields
    const validationErrors = validateForm(formData, selectedSubCategories, !!formData.image)
    ...
    try {
      ...
      const response = await certificationService.sendCertification(submittedData)
      if (response.success) {
        setIsSubmitted(true)
        ...
        showToast(response.error || "Nộp chứng chỉ thất bại.", ToastType.Error)
      }
    } catch (error: any) {
      showToast(error.message || "Không thể nộp chứng chỉ.", ToastType.Error)
    } finally {
        ...
        <DialogContent className="sm:max-w-md">
          <div className="text-center space-y-4 py-4">
            ...
            <div className="space-y-2">
              <h3 className="text-lg font-semibold text-gray-900">Chứng chỉ đã được gửi thành công!</h3>
              <p className="text-sm text-gray-600">
                Chứng chỉ "{formData.name}" của bạn đã được gửi và đang chờ xét duyệt.
              </p>
            </div>
            ...
            <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
              ...
              <p className="text-xs text-blue-600 mt-1">Chúng tôi sẽ xem xét và phản hồi trong vòng 24-48 giờ</p>
            </div>
          </div>
        </DialogContent>
      </Dialog>
    ...
    <Dialog open={open} onOpenChange={handleClose}>
      <DialogContent className="sm:max-w-2xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>Nhập thông tin chứng chỉ để lưu vào hồ sơ chuyên môn của bạn.</DialogDescription>
        </DialogHeader>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="space-y-3">
            <Label>
              ...
              <span className="text-sm text-gray-500 font-normal"> (có thể chọn từ nhiều danh mục khác nhau)</span>
            </Label>
            <div
          ...
          <div className="space-y-1">
            <Label htmlFor="cert-image">
              ...
              <span className="text-sm text-gray-500 font-normal"> (JPEG, PNG, WebP - tối đa 5MB)</span>
            </Label>
            <Input
              ref={fileInputRef}
              id="cert-image"
              ...
              className={errors.file || errors.image ? "border-red-500" : ""}
            />
          ...
          <div className="space-y-1">
            ...
            <div className="flex justify-between items-center">
              ...
              <div className="text-xs text-gray-500 ml-auto">{formData.description.length}/1000</div>
            </div>
          </div>
