   ...
   const handleSave = async () => {
      ...
      try {
         ...
         if (response.success) {
            ...
            showToast(
               response.error || 'Lỗi khi cập nhật chứng chỉ.',
               ToastType.Error
            );
         }
      } catch (error: any) {
         showToast(
            error.message || 'Không thể cập nhật chứng chỉ.',
            ToastType.Error
         );
      } finally {
         setIsSaving(false);
         ...
         <DialogContent className="sm:max-w-2xl max-h-[90vh] overflow-y-auto">
            <DialogHeader>
               ...
               <DialogDescription>
                  ...
                  (typeof certificate.status === 'number' &&
                                          ...
                                          {category.subCategories.map(
                                             (sub, subIndex) => (
                                                ...
                                                >
                                                   ...
                                                   <Badge
                                                      variant="secondary"
                                                      className="text-sm bg-muted/50 text-muted-foreground"
                                                   >
                                                      {sub.name}
                                                   </Badge>
                                                </div>
