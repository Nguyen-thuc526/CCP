...
import {
  Calendar, Building, Edit, Save, X, Award, Clock, AlertCircle,
...
interface ValidationErrors {
  ...
  description?: string;
  categories?: string;
  image?: string; // thêm để báo lỗi ảnh (nếu cần)
}

export default function CertificateDetailModal({
  open, onClose, certificate, onUpdate,
  ...
  // ====== validate file & upload (giống submit) ======
  const validateFile = (file: File): string | undefined => {
    const allowed = ['image/jpeg', 'image/jpg', 'image/png', 'image/webp'];
    const maxSize = 5 * 1024 * 1024; // 5MB
    if (!allowed.includes(file.type)) return 'Chỉ chấp nhận JPG, PNG, WEBP';
    if (file.size > maxSize) return 'Kích thước file không vượt quá 5MB';
    return undefined;
  };
  ...
  const handleImageFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (fileError) {
      setValidationErrors((prev) => ({ ...prev, image: fileError }));
    ...
    uploadImage({
      file,
      onSuccess: (url) => {
        if (editData) setEditData({ ...editData, image: url } as UICertification);
        showToast('Tải ảnh thành công!', ToastType.Success);
      },
      onError: (err) => {
        setValidationErrors((prev) => ({ ...prev, image: 'Upload ảnh thất bại' }));
  ...
  const handleClearImage = () => {
    if (editData) setEditData({ ...editData, image: '' } as UICertification);
  ...
  const handleSave = async () => {
    ...
    try {
      const updateData = {
        ...
        image: editData.image, // ảnh mới (nếu đã upload) sẽ đi vào đây
  ...
  const validateField = (field: keyof ValidationErrors, value: any): string | undefined => {
    switch (field) {
      case 'name':
        ...
        if (value.trim().length < 3) return 'Tên chứng chỉ phải có ít nhất 3 ký tự';
        if (value.trim().length > 100) return 'Tên chứng chỉ không được vượt quá 100 ký tự';
        break;
      case 'description':
        if (!value || value.trim().length === 0) return 'Mô tả là bắt buộc';
        if (value.trim().length < 10) return 'Mô tả phải có ít nhất 10 ký tự';
        if (value.trim().length > 5000) return 'Mô tả không được vượt quá 5000 ký tự';
        break;
      case 'categories':
        if (!selectedSubCategories || selectedSubCategories.length === 0)
          return 'Vui lòng chọn ít nhất một danh mục';
        break;
      // image: không bắt buộc khi update – nếu muốn bắt buộc, mở case này lên.
    }
    return undefined;
  };
  ...
  const getStatusBadge = (status?: string | number) => {
    const statusNum =
      typeof status === 'string'
        ? (status === 'approved' ? CertificateStatus.Active : CertificateStatus.Pending)
        : status;
    switch (statusNum) {
    ...
    <Dialog open={open} onOpenChange={onClose}>
      <DialogContent className="sm:max-w-2xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>
            {isApproved ? 'Thông tin chi tiết về chứng chỉ đã được phê duyệt' : 'Thông tin chi tiết về chứng chỉ đang chờ xét duyệt'}
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="space-y-6">
          ...
          <div className="space-y-2">
            ...
            {isEditing && (
              <div className="space-y-2">
                ...
                <div className="flex items-center gap-2">
                  <Input value={currentData.image || ''} readOnly />
              ...
              <div className="space-y-1">
                <div className="relative">
                  ...
                  <div className="absolute right-3 bottom-3 text-xs text-gray-400">
                    {(currentData.description || '').length}/5000
                  </div>
                </div>
          ...
          <div className="space-y-2">
            ...
            {isEditing ? (
              <div className="space-y-3">
                <div className={`border rounded-lg p-4 bg-gray-50 max-h-60 overflow-y-auto ${validationErrors.categories ? 'border-red-500' : ''}`}>
                  {isLoadingCategories ? (
                              ...
                              <div className="grid grid-cols-1 md:grid-cols-2 gap-2 p-3 bg-blue-50 border border-blue-200 rounded-md">
                                {category.subCategories.map((subCategory) => (
                                  <div key={subCategory.id} className="flex items-center space-x-2">
                                    ...
                                    />
                                    <Label htmlFor={`edit-${subCategory.id}`} className="text-sm font-normal cursor-pointer">
                                      {subCategory.name}
                                    </Label>
                                  </div>
                                ))}
                ...
                {validationErrors.categories && (
                  <p className="text-sm text-red-600 flex items-center gap-1">
                    <AlertCircle className="w-4 h-4" /> {validationErrors.categories}
                  </p>
                )}

                {selectedSubCategories.length > 0 && (
                  <div className="space-y-2">
                    <Label className="text-sm text-muted-foreground">Danh mục đã chọn ({selectedSubCategories.length}):</Label>
                      ...
                      {selectedSubCategories.map((subCategoryId) => {
                        ...
                        return info.name ? (
                          <Badge key={subCategoryId} variant="secondary" className="flex items-center gap-1">
                            ...
                            <span>{info.name}</span>
                            <X className="w-3 h-3 cursor-pointer hover:text-red-500" onClick={() => removeSubCategory(subCategoryId)} />
                          </Badge>
                        ) : null;
                      })}
                    </div>
                  ...
                  (currentData.categories as any[]).map((category: any, index: number) => (
                    <div key={index} className="space-y-2">
                      <div className="flex items-center gap-1">
                        <Badge variant="default" className="text-sm font-medium bg-primary/10 text-primary border-primary/20">
          ...
          <div className="flex justify-between pt-4">
            <div className="flex gap-2">
              {isEditing ? (
                <>
                  <Button variant="outline" size="sm" onClick={handleCancel} disabled={isSaving || uploadingImage}>
                    <X className="w-4 h-4 mr-2" />
                    Hủy
                  </Button>
                  <Button size="sm" onClick={handleSave} disabled={isSaving || uploadingImage}>
