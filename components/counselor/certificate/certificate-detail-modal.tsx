...
export default function CertificateDetailModal({ open, onClose, certificate, onUpdate }: CertificateDetailModalProps) {
  ...
  useEffect(() => {
    if (certificate && isEditing) {
      ...
      setEditData({ ...certificate, categories: certificate.categories || [] })
  ...
  const handleEdit = () => {
    ...
    setIsEditing(true)
    setSelectedSubCategories(certificate.categories?.flatMap((cat) => cat.subCategories.map((sub) => sub.id)) || [])
    setOpenCategories([])
  }
  ...
  const handleSave = async () => {
    ...
    try {
      const updateData = {
        ...
        image: editData.image, // Assuming image is not editable; adjust if it is
  ...
  const currentData = isEditing ? editData! : certificate
      ...
      <DialogContent className="sm:max-w-2xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>
            ...
            (typeof certificate.status === "number" && certificate.status === CertificateStatus.Active)
        ...
        <div className="space-y-6">
          ...
          <div className="space-y-2">
            ...
            {isEditing ? (
              <Input id="cert-name" value={currentData.name} onChange={(e) => handleChange("name", e.target.value)} />
            ) : (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            {certificate.approvedDate && (
              <div className="flex items-center gap-2 text-sm">
                ...
                <span className="font-medium">{new Date(certificate.approvedDate).toLocaleDateString("vi-VN")}</span>
              </div>
            )}
            ...
            {certificate.submittedDate && (
              <div className="flex items-center gap-2 text-sm">
                ...
                <span className="font-medium">{new Date(certificate.submittedDate).toLocaleDateString("vi-VN")}</span>
              </div>
            )}
