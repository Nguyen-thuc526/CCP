...
export default function CertificateManager() {
   ...
   const fetchCertifications = async () => {
      try {
         ...
         if (response.success) {
            ...
            const counts = response.data.reduce(
               (acc, cert) => {
                  ...
                  else if (cert.status === CertificateStatus.Pending)
                     acc.pending++;
                  else if (cert.status === CertificateStatus.NeedEdit)
                     acc.rejected++;
                  return acc;
               },
         ...
         showToast(
            error.message || 'Không thể tải danh sách chứng chỉ.',
            ToastType.Error
         );
   ...
   const approvedCerts = certifications.filter(
      (cert) => cert.status === CertificateStatus.Active
   );
   const pendingCerts = certifications.filter(
      (cert) => cert.status === CertificateStatus.Pending
   );
   const rejectedCerts = certifications.filter(
      (cert) => cert.status === CertificateStatus.NeedEdit
   );
