...
import type { CounselorProfile, UpdateCounselorProfileRequest } from "@/types/user"
...
export function GeneralInfoTab() {
  ...
  const validateField = (field: keyof CounselorProfile, value: string | number): string | null => {
    switch (field) {
      case "fullname":
      ...
      case "phone":
        ...
        if (!/^\d+$/.test(phoneStr)) return "Số điện thoại chỉ được chứa số"
        if (phoneStr.length > 10) return "Số điện thoại không được quá 10 số"
        return null

      case "yearOfJob":
        const experience = Number(value)
        if (isNaN(experience) || experience <= 0) return "Số năm kinh nghiệm phải lớn hơn 1"
        return null

      case "price":
        const price = Number(value)
        if (isNaN(price) || price < 50000) return "Giá tiền phải lớn hơn 50.000 VND"
        return null

      case "description":
        const descStr = String(value).trim()
        if (descStr && descStr.length < 10) return "Mô tả phải có ít nhất 10 ký tự"
        return null
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (profile?.yearOfJob !== undefined) {
        const experienceError = validateField("yearOfJob", profile.yearOfJob)
          ...
          <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
            ...
            <div className="space-y-2">
              ...
              <Input
                ...
                min="1"
                max="50"
                className={validationErrors.yearOfJob ? "border-red-500" : ""}
              />
              {validationErrors.yearOfJob && <p className="text-sm text-red-500">{validationErrors.yearOfJob}</p>}
            </div>
            <div className="space-y-2">
