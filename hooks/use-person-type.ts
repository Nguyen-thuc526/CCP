...
export function usePersonTypes({
   ...
   const categories: Category[] = personTypes.reduce(
      (acc: Category[], personType) => {
         const existingCategory = acc.find(
            (cat) => cat.id === personType.category.id
         );
   ...
   const handleCreate = async (
      ...
      try {
         ...
         const errorMessage =
            err instanceof Error
               ? err.message
               : 'Có lỗi xảy ra khi tạo loại tính cách';
         showToast(errorMessage, ToastType.Error);
         return false;
      }
   };
   const handleUpdate = async (
      ...
      try {
         ...
         const errorMessage =
            err instanceof Error
               ? err.message
               : 'Có lỗi xảy ra khi cập nhật loại tính cách';
         showToast(errorMessage, ToastType.Error);
         return false;
      }
   };
